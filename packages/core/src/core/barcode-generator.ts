...
import { validateGenerationEnvironment, getRecommendedRenderType } from './env-detector';
...
class BarcodeInstanceImpl implements BarcodeInstance {
  ...
  constructor(config: BarcodeConfig) {
    ...
    const envCheck = validateGenerationEnvironment();
    if (!envCheck.supported) {
      throw new Error(`Environment not supported for barcode generation: ${envCheck.issues.join(', ')}`);
    }

    // Create renderer with fallback
    let renderType = config.renderType || DEFAULT_CONFIG.renderType;
    const recommendedType = getRecommendedRenderType();
    
    try {
      this.renderer = renderType === 'svg' ? new SVGRenderer() : new CanvasRenderer();
    } catch (error) {
      // Fallback to recommended type if initial type fails
      if (recommendedType && recommendedType !== renderType) {
        renderType = recommendedType;
        this.renderer = renderType === 'svg' ? new SVGRenderer() : new CanvasRenderer();
      } else {
        throw error;
      }
